...
import { Menu, X, Scissors, LogOut, User, ChevronDown, LayoutDashboard } from "lucide-react";
import { navigationLinks } from "../../data/public/navigation.data";
...
export default function Navbar({ user }: Props) {
  ...
  useEffect(() => {
    ...
    if (params.get('login') === 'true') {
      ...
      setIsLoginModalOpen(true);
      setLoginError("No tienes permisos de administrador para acceder a esa sección.");
    }
  }, []);
        ...
        <div className="flex items-center gap-3">
          ...
          {user ? (
            <div className="relative group">
              ...
              {/* Dropdown Menu */}
              <div className={`absolute right-0 mt-2 w-48 bg-surface border border-surface shadow-xl rounded-2xl py-2 z-[60] transition-all transform origin-top-right ${userMenuOpen ? 'scale-100 opacity-100' : 'scale-95 opacity-0 pointer-events-none'}`}>
                <div className="px-4 py-2 border-b border-bg/50 mb-1">
                  <p className="text-xs text-text/40 font-medium uppercase tracking-wider">Cuenta</p>
                </div>
                
                <a href="/profile" className="flex items-center gap-3 px-4 py-2 text-sm text-text/70 hover:text-primary hover:bg-primary/5 transition-colors">
                  <User className="w-4 h-4" />
                  Mi Perfil
                </a>

                {user.isAdmin && (
                  <a href="/admin" className="flex items-center gap-3 px-4 py-2 text-sm text-amber-500 hover:bg-amber-500/5 transition-colors font-semibold">
            ...
            <button
              ...
              className="hidden md:inline-flex bg-primary hover:bg-primary-hover text-white text-sm font-semibold px-5 py-2 rounded-full transition-colors"
            >
              Iniciar sesión
            </button>
          )}
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
          ...
          <div className="relative bg-surface border border-surface w-full max-w-md rounded-3xl p-8 md:p-10 shadow-2xl transform transition-all animate-in fade-in zoom-in duration-300">
            ...
            <div className="text-center">
              ...
              <p className="mt-8 text-[10px] text-text/40 px-6 uppercase tracking-widest font-medium">
