...
import { Scissors, MapPin, Phone, Clock, MessageCircle } from "lucide-react";
...
export default function Footer({ navLinks, contactData }: FooterProps) {
  ...
  return (
    <footer
      className="bg-stone-950 border-t border-stone-900 pt-24 pb-12 px-6"
    >
      <div className="max-w-7xl mx-auto">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
              <div className="space-y-6">
                <div className="flex items-start gap-4 text-sm">
                  ...
                  <span className="text-stone-500 font-medium leading-relaxed">{contactData.address}</span>
                </div>
                <div className="flex items-center gap-4 text-sm">
                  <div className="p-2 bg-amber-500/10 rounded-lg shrink-0">
                    <Phone className="w-4 h-4 text-amber-500" />
                  </div>
                  <span className="text-stone-500 font-bold">{contactData.phone}</span>
                </div>
              </div>
                ...
                <div className="flex flex-col gap-3">
                  {(contactData?.schedule || []).map((entry) => (
                    <div key={entry.days} className="flex flex-col">
                      <span className="text-2xs font-black text-stone-700 uppercase tracking-ultra">{entry.days}</span>
                      <span className="text-stone-400 font-bold tracking-wider">{entry.hours}</span>
                    </div>
                  ))}
                </div>
              </div>
        ...
        <div className="border-t border-stone-900 pt-10 flex flex-col sm:flex-row items-center justify-between gap-6 text-2xs font-black text-stone-700 uppercase tracking-mega">
          ...
          <p className="flex items-center gap-2">
            Hecho con <span className="text-red-500 text-lg">♥</span> en México
          </p>
        </div>
      </div>
    </footer>
